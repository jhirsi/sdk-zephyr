...
static void w5500_rx(const struct device *dev)
{
	...
	do {
		...
		if (!pkt_buf) {
			/* Still discard the frame, or w5500_rx will spin on it forever. */
...
static void w5500_poll_service(const struct device *dev)
{
	...
	/* Gate on RX_RSR, not S0_IR.RECV: the IR bit may not re-assert for
	 * frames already buffered before a prior burst-capped pass.
	 */
	w5500_spi_read(dev, W5500_S0_RX_RSR, rsr, 2);
	if (sys_get_be16(rsr) == 0U) {
		return;
	}

	/* SENDOK is handled by w5500_tx_wait_sendok(); only clear RECV here. */
	w5500_spi_read(dev, W5500_S0_IR, &ir, 1);
	ir &= S0_IR_RECV;
	if (ir != 0U) {
		w5500_spi_write(dev, W5500_S0_IR, &ir, 1);
