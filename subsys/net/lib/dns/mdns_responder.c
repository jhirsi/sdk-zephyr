		   ...
		   struct sockaddr *dst, socklen_t *dst_len)
{
	...
	if (IS_ENABLED(CONFIG_NET_IPV4) && family == AF_INET) {
		...
		*dst_len = sizeof(struct sockaddr_in6);
		...
		if (ret < 0) {
			NET_ERR("Cannot set %s multicast %s (%d)", "IPv6", "hoplimit", ret);
		}
	} else {
		return -EPFNOSUPPORT;
	}
		    ...
		    size_t addrlen)
{
	...
	do {
		...
		if (!strncasecmp(hostname, result->data + 1, hostname_len) &&
			...
			/* TODO Jani mun mielestä viimesin ehto on väärä koska hostname_len sisältää .local */
			NET_DBG("%s query to our hostname %s.local", "mDNS", hostname);
			...
			send_sd_response(sock, family, src_addr, addrlen, &dns_msg, result);
