...
int net_route_packet(struct net_pkt *pkt, struct net_in6_addr *nexthop)
{
	...
	/* Switch to the outbound iface before setting link-layer addresses so the
	 * source address is taken from the nexthop iface, not the inbound one.
	 */
	net_pkt_set_iface(pkt, nbr->iface);

	/* Set the source ll address of the outbound iface (if relevant) and the
