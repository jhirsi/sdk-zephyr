...
/*
 * net_route_packet() must set the link-layer source address from the
...
ZTEST(route_packet_suite, test_route_outbound_ll_src)
{
	...
	ret = net_route_packet(pkt, &rp_nexthop);
	zassert_ok(ret, "net_route_packet failed (%d)", ret);
