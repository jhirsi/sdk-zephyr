 ...
 *
 * DNS socket dispatcher pairing on a dual-homed host (no mDNS):
...
static struct dual_if_test if1_data;
static struct dual_if_test if2_data;

static int dual_if_dummy_send(const struct device *dev, struct net_pkt *pkt)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(pkt);
...
static uint8_t *dual_if_mac(struct dual_if_test *data, uint8_t idx)
{
...
{
	net_if_set_link_addr(iface, dual_if_mac(&if1_data, 1),
			     sizeof(struct net_eth_addr), NET_LINK_ETHERNET);
}

static void if2_init(struct net_if *iface)
{
	net_if_set_link_addr(iface, dual_if_mac(&if2_data, 2),
			     sizeof(struct net_eth_addr), NET_LINK_ETHERNET);
}

#define _DUAL_L2 DUMMY_L2
#define _DUAL_L2_CTX NET_L2_GET_CTX_TYPE(DUMMY_L2)

static struct dummy_api if1_api = {
	.iface_api.init = if1_init,
	.send = dual_if_dummy_send,
};
...
static void *dual_iface_setup(void)
{
...
/* Multi-interface host: an unscoped (ifindex 0) resolver must not pair with a
 * scoped responder. "Any interface" is ambiguous here — pairing would silently
 * drop traffic on the other interfaces.
 */
ZTEST(dns_dispatch_pair, test_dispatcher_unscoped_refuses_dual_iface)
{
	int ret;
	...
	zassert_is_null(resp1.pair, "unscoped resolver must not pair on dual-iface");
	zassert_is_null(resv_any.pair, "unscoped resolver must stay unpaired");

	teardown_primary(&resv_any, &resv_any_fd);
	teardown_primary(&resp1, &resp1_fd);
}

ZTEST_SUITE(dns_dispatch_pair, NULL, dual_iface_setup, NULL, NULL, NULL);
